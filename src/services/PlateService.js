// Servicio: PlateService.js
// Servicio para gestionar platos, alérgenos, categorías y ofertas en Firestore.
  ...
  async createPlate(plateData, isAdmin = false) {
    try {
      if (!isAdmin) {
        return {
          success: false,
          error: "Solo administradores pueden crear platos",
        };
      }
  ...
  async updatePlate(id, plateData, isAdmin = false) {
    try {
      if (!isAdmin) {
        return {
          success: false,
          error: "Solo administradores pueden actualizar platos",
        };
      }
  ...
  async deletePlate(id, isAdmin = false) {
    try {
      if (!isAdmin) {
        return {
          success: false,
          error: "Solo administradores pueden eliminar platos",
        };
      }
